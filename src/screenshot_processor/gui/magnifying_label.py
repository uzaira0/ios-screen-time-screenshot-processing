...
class MagnifyingLabel(QLabel):
    ...
    def mouseMoveEvent(self, ev: QMouseEvent | None) -> None:
        ...
        try:
            ...
            rel_x = (x - left) / width
            rel_y = (y - top) / height
            crosshair_x = int(x_offset + rel_x * magnified_pixmap.width())
            crosshair_y = int(y_offset + rel_y * magnified_pixmap.height())

            crosshair_length = int(10 * self.screen_scale_factor)
            painter.drawLine(
                crosshair_x - crosshair_length,
                crosshair_y,
                crosshair_x + crosshair_length,
                crosshair_y,
            )
            ...
            logger.debug(f"Error creating magnifier: {e}")
        except Exception as e:
            logger.error(f"Unexpected error in magnifier: {e}")
            raise
